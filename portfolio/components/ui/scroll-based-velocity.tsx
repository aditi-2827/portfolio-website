...
const ScrollVelocityContext = React.createContext<any>(null);
...
export function ScrollVelocityContainer({
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      if (isAtBottom && e.deltaY > 0) {
        setVirtualScrollY((prev) => prev + e.deltaY);
      } else if (scrollTop > 0) {
        setVirtualScrollY(0);
      }
    };
  ...
  const combinedScrollY = useTransform(scrollY, (v) => {
    return v + virtualScrollY;
  });

  const combinedVelocity = useVelocity(combinedScrollY);
  const smoothVelocity = useSpring(combinedVelocity, {
    damping: 50,
    stiffness: 400,
  });
...
export function ScrollVelocityRow(props: any) {
...
function ScrollVelocityRowLocal(props: any) {
  const { scrollY } = useScroll();
